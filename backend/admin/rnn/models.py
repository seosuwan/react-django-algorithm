...
class MyRNN(object):
    ...
    def ram_price(self):
        ...
        x_train = train['date'][:, np.newaxis]  # 모든행에 새로운 열을 만들어
        y_train = np.log(train['price'])
        tree = DecisionTreeRegressor().fit(x_train, y_train)
        lr = LinearRegression().fit(x_train, y_train)
        x_all = ram_price['date'].values.reshape(-1, 1)  # 행은 그대로 놔두고 열은 하나만// 하나의열에 여러개
        ...
        plt.savefig(f'{self.vo.context}ram_prediction.png')
